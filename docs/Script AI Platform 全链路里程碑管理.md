...
| M4 | TipTap 全模块接入（结构增强） | 保存 ≥50 次无损坏 | 已完成 |
...
- 线上已验证：lock -> unlock -> truth 状态回到 DRAFT
...
**验收证据**：
- 生产 D1 已执行 `d1-schema.sql`，`module_documents` 表存在
- 模块 PUT→GET 回读一致（overview/roles/clues/timeline/dm）
- 基线回归脚本 20/20 通过（见 `docs/ops/online-acceptance-report.md`）
...
## Milestone 7 — 真实模型接入（生成角色/线索/时间线/DM/概览）
**目标**：角色/线索/时间线/DM/概览生成可重复、可编辑。  
**Gate 7**：可重复生成 10 次不崩溃；覆盖/追加策略明确。  
**验收方式**：角色/线索/时间线/DM/概览多次生成回归。  
**产出物**：角色/线索/时间线/DM/概览生成与覆盖策略文档。
