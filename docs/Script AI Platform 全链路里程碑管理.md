...
**状态**：进行中（Gate 8.5 待验收；证据见 `docs/ops/online-acceptance-report.md`）

定位  
在进入社区（M9）前，对编辑器进行一次集中可用性修复与交互打磨，目标是“能稳定写、能稳定切、不会崩、预览一致、交互顺手”。

目标（Objective）
- 修复锁定/解锁 Truth 引发的前端崩溃（removeChild NotFoundError）
- 修复条目切换后卡住/无响应问题
- 将多条目导航改为可折叠嵌套结构（默认折叠，当前模块自动展开）
- 删除所有浏览器 prompt/confirm 作为核心交互（改为内联编辑/确认）
- 修复预览页内容一致性（展示 Overview 真实内容）

交付内容（Deliverables）
- 侧栏嵌套导航：Roles/Clues/Timeline/DM 子条目可折叠，新增条目后自动进入内联编辑
- 内联重命名：Enter 保存 / Esc 取消 / 失焦保存或取消
- 锁定/解锁 Truth：前端不崩溃，编辑区只读、可恢复编辑
- 条目切换稳定：编辑器不会冻结，右侧面板可交互
- 预览页一致：展示 Overview 文档内容或项目简介
- Debug 开关：`NEXT_PUBLIC_EDITOR_DEBUG=true` 可输出 editor create/destroy 与 suggestion 生命周期

Gate 8.5（完成条件）
1) 真相锁定/解锁连续 20 次无崩溃（含锁定后切模块 → 解锁 → 再编辑）
2) 条目切换稳定：roles/clues 各 ≥20 次切换无卡死
3) 预览一致：预览页 Overview 文本与编辑器一致（≥3 个项目抽测）
4) 内联重命名：roles/clues/timeline/dm 各 1 次新增 → 改名 → 刷新一致
5) verify-online 基线 20/20 通过

验收方式（Acceptance）
- 生产环境按 Gate 8.5 步骤手工走查
- 运行 verify-online.ps1 / verify-online.sh
- 证据与复现路径写入 online-acceptance-report.md

产出物
- 编辑器可用性修复代码
- Gate 8.5 线上验收记录
- 交互说明文档（如 `docs/frontend/editor-m8.5-ux.md`）
