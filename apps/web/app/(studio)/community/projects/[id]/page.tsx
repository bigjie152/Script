...
export default function CommunityProjectDetailPage() {
  ...
  const overviewText = useMemo(() => {
    ...
    const text = extractTextFromContent(data.overview);
    return text || "";
  }, [data?.overview]);
  ...
  const summary =
    data.project.communitySummary && typeof data.project.communitySummary === "object"
      ...
      : null;
  const genre =
    getStringValue(meta?.genre) || getStringValue(summary?.genre) || "题材未设置";
  const players =
    getStringValue(meta?.players) || getStringValue(summary?.players) || "人数未知";
  const tag =
    getFirstString(meta?.tags) || getFirstString(summary?.tags) || "多线叙事";
  const introText =
    ...
    <div className="max-w-6xl mx-auto px-6 py-6 space-y-6">
      <div className="flex flex-wrap items-center gap-3 text-sm text-slate-500">
        <button
          ...
          className="rounded-full border border-slate-200 bg-white px-3 py-1 text-sm text-slate-900"
        >
          ← 返回
        </button>
        <div className="text-lg font-semibold text-slate-900">{data.project.name}</div>
        <span className="text-xs text-slate-400">by {data.author.username}</span>
      </div>

      <section className="bg-white rounded-2xl border border-slate-100 shadow-sm px-6 py-6">
        <div className="grid gap-6 lg:grid-cols-[220px_minmax(0,1fr)]">
          ...
          <div className="space-y-4">
            <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
              <span className="rounded-full border border-slate-200 bg-white px-3 py-1">{genre}</span>
              <span className="rounded-full border border-slate-200 bg-white px-3 py-1">{players}</span>
              <span className="rounded-full border border-slate-200 bg-white px-3 py-1">{tag}</span>
            </div>
            <div className="text-sm text-slate-500">{introText}</div>
            <div className="flex flex-wrap items-center gap-2 text-xs">
              <span
                className={`rounded-full px-3 py-1 ${
                  aiPass ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600"
                }`}
              >
              ...
              <span
                className={`rounded-full px-3 py-1 ${
                  isLocked ? "bg-indigo-50 text-indigo-600" : "bg-slate-100 text-slate-500"
                }`}
              >
      ...
      <div className="flex flex-wrap items-center gap-3">
        {data.isOwner ? (
          <button
            ...
            className="rounded-full bg-indigo-600 px-4 py-2 text-sm text-white shadow-sm"
          >
            进入编辑器
          </button>
        ) : null}
      ...
      <section className="bg-white rounded-2xl border border-slate-100 shadow-sm px-6 py-5">
        <div className="flex items-center gap-6 border-b border-slate-100 pb-4 text-sm">
          <button
            ...
            className={`text-sm font-semibold ${
              activeTab === "comments" ? "text-indigo-600" : "text-slate-500"
            }`}
        ...
        {activeTab === "overview" ? (
          <div className="mt-6">
            <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50/60 px-4 py-6 text-sm text-slate-500 whitespace-pre-wrap">
              {overviewText || "暂无概览内容"}
            </div>
          </div>
        ) : (
          <div className="mt-6 space-y-6">
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-end gap-3">
                <button
                  ...
                  disabled={busy}
                  className="rounded-full bg-indigo-600 px-4 py-2 text-sm text-white shadow-sm disabled:opacity-60"
                >
                  发布
                </button>
              </div>
            ...
            <div className="space-y-4">
              {data.comments.length === 0 ? (
                <div className="text-xs text-slate-400">暂无评论</div>
              ) : (
                  ...
                  return (
                    <div key={comment.id} className="rounded-2xl border border-slate-100 bg-white px-4 py-4">
                      ...
                      {editingId === comment.id ? (
                        ...
                        <div className="mt-3 text-sm text-slate-900">{comment.content}</div>
                      )}
                      <div className="mt-4 flex flex-wrap items-center gap-2 text-xs text-slate-500">
                        ...
                        <button type="button" className="text-indigo-600" onClick={() => setReplyTo(comment.id)}>
                          回复
                        </button>
                        ...
                        {(user?.id === comment.userId || data.isOwner) ? (
                          <button type="button" className="text-rose-600" onClick={() => handleDelete(comment.id)}>
                            删除
                          </button>
                        ) : null}
                        {data.isOwner && comment.isSuggestion && comment.status !== "accepted" ? (
                          <button type="button" className="text-emerald-600" onClick={() => handleAccept(comment.id)}>
                            采纳
                          </button>
                        ) : null}
                      </div>
                      ...
                      {visibleReplies.length > 0 ? (
                        <div className="mt-4 space-y-2 border-l border-slate-200 pl-3">
                          {visibleReplies.map((reply) => (
                            <div key={reply.id} className="text-xs text-slate-500">
                              <div className="flex items-center justify-between">
                                <span>{reply.username}</span>
                                <span>{reply.createdAt}</span>
                              </div>
                              <div className="mt-1 text-sm text-slate-900">{reply.content}</div>
                            </div>
                          ))}
                          {replies.length > 2 ? (
                            <button
